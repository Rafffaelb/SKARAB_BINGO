...
\quad O processo de transformar um modelo matemático em um hardware funcional na FPGA da SKARAB envolve duas etapas principais: a geração do arquivo de configuração (bitstream) e a transferência deste para a placa.
...
\caption{Fluxo de trabalho para a plataforma SKARAB, desde a modelagem em Simulink até a programação da FPGA.}
...
\subsection{O espectrômetro do BINGO e desafios}
    ...
    \quad No desenvolvimento do backend digital para o projeto BINGO, a placa SKARAB foi configurada com sucesso para operar como um espectrômetro em tempo real com largura de banda de 187,5~MHz e resolu\c{c}\~ao de 32.768 bines de frequ\^encia. Esta configura\c{c}\~ao baseia-se no \textit{toolflow} CASPER, integrando blocos de FFT (\textit{Fast Fourier Transform}) de alto desempenho e acumuladores vetoriais. A valida\c{c}\~ao do sistema incluiu a identifica\c{c}\~ao da faixa din\^amica livre de esp\'urios (SFDR) e a verifica\c{c}\~ao da integridade do sinal em condi\c{c}\~oes de laborat\'orio.
    ...
    Os pr\'oximos passos para o espectr\^ometro incluem a expans\~ao da largura de banda para 375~MHz (utilizando dual-ADC) e a redu\c{c}\~ao do n\'umero de bines para 8.192 para otimizar o uso de recursos de mem\'oria BRAM na FPGA. Al\'em disso, estamos desenvolvendo blocos de IP para o gerenciamento de metadados via protocolo SPEAD, garantindo a rastreabilidade temporal dos dados atrav\'es de \textit{timestamps} de alta precis\~ao sincronizados via sinal PPS (\textit{Pulse Per Second}).
